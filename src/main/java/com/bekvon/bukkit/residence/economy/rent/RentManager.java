...
public class RentManager implements MarketRentInterface {
    private final Set<ClaimedResidence> rentedLand;
    private final Set<ClaimedResidence> rentableLand;
...
//	return (((long) days) * 1000L);
    ...
    public boolean getRentableRepeatable(ClaimedResidence res) {
        if (res == null)
            return false;
        return res.isForRent() && res.getRentable().AllowRenewing;
    }
    ...
    public boolean getRentedAutoRepeats(ClaimedResidence res) {
        if (res == null)
            return false;
        return getRentableRepeatable(res) && (rentedLand.contains(res) && res.getRentedLand().AutoPay);
    }
